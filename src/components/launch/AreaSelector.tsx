...
export function AreaSelector() {
	...
	const render = useCallback(
		(currentSelection = selection) => {
			...
			if (currentSelection) {
				...
				const labelY =
					currentSelection.y < 30
						? currentSelection.y + currentSelection.height + 25
	...
	const confirmSelection = useCallback(async () => {
		if (isSelectionTooSmall(selection)) {
			toast.error(
				t(
					"sourceSelector.selectionTooSmall",
					"Selection area is too small. Please select an area at least 100x100 pixels.",
				),
			);
			return;
		}
		...
		if (selection) {
			...
			if (result.success) {
				...
				toast.error(
					result.error || result.message || "Failed to set recording area. Please try again.",
				);
			}
		}
	}, [selection, t]);
