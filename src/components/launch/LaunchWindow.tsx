...
export function LaunchWindow() {
	...
	const [selectedSource, setSelectedSource] = useState<DesktopSource>({
		id: "default",
		name: "Screen",
		thumbnail: null,
		display_id: "",
	...
	const idleControls = (
		<>
			...
			>
				...
				{selectedSource.id === "area:custom" && (
					<span className="text-[9px] font-black bg-blue-600 text-white px-1 rounded ml-1 scale-90">
						CROP
					</span>
				)}
				<ChevronUp
									...
									{sourcesLoading ? (
										...
										<>
											...
											<div className="flex flex-col shrink-0">
												<div className={styles.ddLabel} style={{ paddingBottom: 6 }}>
													Recording Mode
												</div>
														...
														<button
															onClick={() => {
																...
																window.electronAPI.openAreaSelector({
																	displayId: screenSrc?.display_id,
																});
																setActiveDropdown("none");
															}}
															className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-[9px] text-[10px] font-bold tracking-wider uppercase transition-all duration-200 ${
												...
												<>
													...
													{screenSources.map((source) => {
														...
														const isParentScreen =
															isAreaMode && lastScreenSourceRef.current?.id === source.id;
														return (
															<DropdownItem
																key={source.id}
