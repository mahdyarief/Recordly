  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const handleGlobalMouseUp = (upEvent: MouseEvent) => {
        if (isDraggingSelectionRef.current && selectionAnchorMsRef.current !== null) {
          const moveX = upEvent.clientX - capturedRect.left - sidebarWidth;
          const moveRelativeMs = pixelsToValue(moveX);
          const moveAbsoluteMs = Math.max(0, Math.min(range.start + moveRelativeMs, videoDurationMs));
          const start = Math.min(selectionAnchorMsRef.current, moveAbsoluteMs);
          onSeek?.(start / 1000);
          selectionAnchorMsRef.current = Math.max(selectionAnchorMsRef.current, moveAbsoluteMs);
        }
...
export default function TimelineEditor({
  ...
  const handleAddZoom = useCallback(() => {
    ...
    if (isOverlapping || gapToNext <= 0 || duration > gapToNext) {
  ...
  const handleAddAudio = useCallback(async () => {
    ...
    const actualDuration = timeSelection ? (timeSelection.endMs - timeSelection.startMs) : Math.min(audioDurationMs, gapToNext, totalMs - startPos);
    onAudioAdded({ start: startPos, end: startPos + actualDuration }, result.path);

    if (timeSelection) {
      onTimeSelectionChange?.(null);
    }
  }, [videoDuration, totalMs, currentTimeMs, audioRegions, onAudioAdded, timeSelection, onTimeSelectionChange]);
  ...
  const timelineItems = useMemo<TimelineRenderItem[]>(() => {
    ...
    const videoAudio: TimelineRenderItem[] = videoPath && totalMs > 0 ? [{
      id: `original-video-audio-${totalMs}`,
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col bg-[#17171a] overflow-auto">
      <div className="flex items-center gap-4 px-4 py-2 border-b border-white/10 bg-[#161619]">
        <div className="flex items-center bg-black/40 rounded-lg p-1 border border-white/5 shadow-inner backdrop-blur-sm">
          <Button
            onClick={() => onTimelineModeChange?.('move')}
            variant="ghost"
            size="icon"
            className={cn(
              "h-7 w-9 transition-all rounded-md border border-transparent flex items-center justify-center relative",
              timelineMode === 'move'
                ? "bg-gradient-to-b from-blue-500 to-blue-600 text-white shadow-[0_0_12px_rgba(37,99,235,0.4)] border-white/20"
                : "text-white/40 hover:text-white/80 hover:bg-white/5"
            )}
            title="Move Tool (V)"
          >
            <MousePointer2 className={cn("w-3.5 h-3.5", timelineMode === 'move' ? "drop-shadow-[0_0_3px_rgba(255,255,255,0.4)]" : "")} />
          </Button>
          <Button
            ...
            title="Select Tool (E)"
          >
            <BoxSelect className={cn("w-3.5 h-3.5", timelineMode === 'select' ? "drop-shadow-[0_0_3px_rgba(255,255,255,0.4)]" : "")} />
          </Button>
        </div>
      ...
      >
        ...
        >
          ...
          <Timeline
            ...
            onMasterAudioVolumeChange={onMasterAudioVolumeChange}
            onAudioTrackVolumeChange={onAudioTrackVolumeChange}
