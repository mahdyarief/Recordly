  ...
  return (
    <div
      ...
      onMouseDown={(e) => {
        ...
        // and background seeking in both modes if we click the item.
        e.stopPropagation();
      }}
      onClick={(e) => {
    ...
    >
      ...
      <div className="h-full" style={{ ...itemContentStyle, minWidth: 24, height: "100%" }}>
        <div
          ...
          style={{ height: "100%", width: "100%", minHeight: 22, color: '#fff', minWidth: 24 }}
          onClick={(event) => {
            // Prevent the timeline background's onClick (seeking) from firing
        ...
        >
          ...
          {isResizable && (
            <>
              <div
                ...
                style={{
                  cursor: timelineMode === 'move' ? 'col-resize' : 'default',
                  pointerEvents: timelineMode === 'move' ? 'auto' : 'none',
                  width: 8, opacity: 0.9, background: endCapColor
                }}
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-white/90 opacity-80 group-hover:opacity-100 transition-opacity select-none overflow-hidden max-w-full">
            ...
            <span
              className={`text-[9px] tabular-nums tracking-tight whitespace-nowrap transition-opacity ${isSelected ? 'opacity-60' : 'opacity-0 group-hover:opacity-40'
                }`}
            >
              {timeLabel}
            </span>
