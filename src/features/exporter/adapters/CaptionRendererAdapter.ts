...
} from "@/features/captions/domain/entities/captionLayout";
import {
	CAPTION_FONT_WEIGHT,
	CAPTION_LINE_HEIGHT,
	getCaptionPadding,
...
import { drawSquircleOnCanvas } from "@/shared/lib/squircle";
...
export function renderCaptions(
	...
	const maxMeasuredWidth = activeCaptionLayout.visibleLines.reduce(
		(largest: number, line: CaptionLineLayout) => Math.max(largest, line.width),
		0,
	);
	...
	activeCaptionLayout.visibleLines.forEach((line: CaptionLineLayout, lineIndex: number) => {
