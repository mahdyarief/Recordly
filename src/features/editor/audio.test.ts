...
import { AudioRegion } from "../project/domain/entities/AudioRegion";
import { ProjectEditorState } from "../project/domain/entities/ProjectState";
...
describe("Audio region normalization", () => {
	describe("volume is clamped to [0, 2]", () => {
		it("should clamp volume > 2 down to 2", () => {
				...
				audioRegions: [
					{
						...
						audioPath: "/test.mp3",
						volume: 5,
					} as unknown as AudioRegion,
				],
			} as Partial<ProjectEditorState>);
			expect(result.audioRegions[0].volume).toBe(2);
		});
		...
		it("should preserve valid volume values in [0, 2]", () => {
			fc.assert(
				fc.property(fc.double({ min: 0, max: 2, noNaN: true }), (volume) => {
					const result = normalizeProjectEditor({
						audioRegions: [
							{
								id: "audio-1",
	...
	describe("empty or missing audioRegions", () => {
		it("should return empty array when audioRegions is undefined", () => {
			const result = normalizeProjectEditor({
				audioRegions: null as unknown as AudioRegion[],
			} as Partial<ProjectEditorState>);
		...
		it("should return empty array when audioRegions is not an array", () => {
			const result = normalizeProjectEditor({
				audioRegions: "invalid" as unknown as AudioRegion[],
			} as Partial<ProjectEditorState>);
			expect(result.audioRegions).toEqual([]);
		});
