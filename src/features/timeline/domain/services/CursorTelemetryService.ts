import { TrimRegion } from "@/features/project/domain/entities/TrimRegion";
import { clamp } from "@/shared/lib/math";
import { CursorTelemetryPoint } from "../entities/TimelineTypes";
...
function findFirstStableCursorType(samples: CursorTelemetryPoint[]) {
	for (const sample of samples) {
		...
		if (
			...
			sample.interactionType === "contextmenu" ||
	...
	return samples[0]?.cursorType ?? "pointer";
}
...
function findLatestStableCursorType(samples: CursorTelemetryPoint[], timeMs: number) {
	...
	return findLatestSample(samples, timeMs)?.cursorType ?? "pointer";
}
